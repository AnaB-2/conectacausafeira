...
async function Post(payload) {
  try {
    ...
    const usuarioExistente = await Login(payload.email);

    if (usuarioExistente) {
      return {
        message: "Error",
        data: "Este email já está cadastrado!"
      };
    }
    ...
    console.error("Erro no usersModel Post:", error);
...
async function Login(email) {
  const sqlText = `SELECT * FROM users WHERE email = ?`;
  const [result] = await db.execute(sqlText, [email]);

  return result.length > 0 ? result[0] : null;
}

module.exports = { Get, GetById, Post, Put, Delete, EndPointName, Login}